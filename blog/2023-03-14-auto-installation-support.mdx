...
title: Adding auto-installation support
description:
  Although it was in our minds from the beginning, we have been postponing the support for
  auto-installation for several reasons. Now the time has come and we have a plan.
tags:
  - auto-installation
---
...
In that regard, the _system definition_ for auto-installation is another user interface. The D-Bus
...
## Replacing XML with Jsonnet {#jsonnet}

Although you might have your own list, there are a few things we did not like about AutoYaST
profiles:
...
But making use of Jsonnet features, we can also have dynamic profiles, replacing rules/classes or
ERB with a single and better alternative for this purpose. Let's have a look to a complex profile:
...
This Jsonnet file is evaluated and validated at installation time, generating a JSON profile where
the `findBiggestDisk(dinstaller.disks)` is replaced with the name of the biggest disk.

Beware that defining
[the format](https://github.com/imobachgs/dinstaller-rs/blob/main/dinstaller-lib/share/profile.schema.json)
...
report the installation progress. Again through our CLI, we plan to solve that problem by providing
such a mechanism which, to be honest, is already available in many configuration systems.
