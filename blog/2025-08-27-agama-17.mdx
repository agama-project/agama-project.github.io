...
There is also a new option to re-scan the system in case new hardware devices has been plugged, new
logical devices (line RAIDs or LVM volume groups) has been created or the user needs a second chance
to introduce an encryption password.
...
## New options at the JSON configuration {#unattended}
...
But sometimes it is Agama who wants to ask something to the user. For example, when it finds an
...
Users of unattended installation will of course benefit from the mentioned new options added to the
JSON configuration. But the improvements for automated installations go beyond that.

On the one hand, Agama does not longer proceed silently when it fails to fetch or process a
configuration provided via the `inst.auto` boot argument. Instead, this new version displays an
error message asking the user whether to retry. If the user decides to not retry, Agama does not
...
installation. That simplifies setting up the corresponding infrastructure at controlled
environments.
...
As you know, Agama allows to install different distributions that are called "products" at Agama's
...
will know find a new option at the initial page that allows to select the distribution to install.
...
the content a lot and it was not feasibly to define a redirection for every moved piece of
