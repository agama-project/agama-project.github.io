...
- `status` — Prints the current state of the installation (e.g., waiting, blocked, running, or
  finished)
...
- `<METHOD>` — What to do after finishing the installation. Possible values:
  ...
  reboot - reboot into the installed system. This value is the default. It can be overriden by
  setting the inst.finish kernel command-line argument.
...
- `--format <FORMAT>` — Specify in which format status will be shown
  ...
  - `text`: textual format that is optimized to be read for humans, can change in future and can be
    localized
