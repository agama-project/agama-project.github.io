...
- Different components use different commands for building each POT file
  - The web frontend runs the
    [web/build_pot](https://github.com/agama-project/agama/blob/master/web/build_pot) script
  ...
  - The Rust part of the service backend uses
    [rust/build_pot](https://github.com/agama-project/agama/blob/master/rust/build_pot)
...
- Product translations:
  ...
  - Runs the
    ...
    script which injects the translations from the PO files into the product YAML files.
...
- Rust backend translations:
  [weblate-merge-rust-po.yml](https://github.com/agama-project/agama/blob/master/.github/workflows/weblate-merge-rust-po.yml)
  - Just copies the PO files to the `rust/po` directory
...
translation component for each Agama part (the web front-end, the Ruby D-Bus back-end, the HTTP Rust
back-end, and product definitions).
...
[languages.json](https://github.com/agama-project/agama/blob/master/web/src/languages.json). Such a
list is automatically refreshed when any of the PO files is updated (see
[Uploading Translatable Texts](#uploading-translatable-texts)).

The
[update-manifest.py script](https://github.com/agama-project/agama/blob/master/web/share/update-manifest.py)
is the responsible for updating the list of supported languages. It goes through all PO files,
filters out the ones with less than 70% of translated content and adds the corresponding locales to
the `languages.json`.
...
`es_es`), the script uses the
[locales.json map](https://github.com/agama-project/agama/blob/master/web/share/locales.json) to
determine which territory to use.
...
[agama-weblate](https://github.com/agama-project/agama-weblate) repository checkout. You have to
clone it manually, in the Agama checkout root directory run command:
...
The [i18n module](https://github.com/agama-project/agama/blob/master/web/src/i18n.js) offers a set
...
specific number. See more details about the
[plural forms](https://www.gnu.org/software/gettext/manual/html_node/Plural-forms.html) in the GNU
...
### Rust: Marking texts for translation

We are using [gettext-rs][] and [i18n-format][].

[gettext-rs]: https://docs.rs/gettext-rs
[i18n-format]: https://docs.rs/i18n-format

Example:

```rust
use gettextrs::gettext;
use agama_l10n::helpers::gettext_noop; // does nothing but seen by build_pot
use i18n_format::i18n_format;

// TRANSLATORS: disk volume, not audio
let s = gettext("Volume");

let id = gettext_noop("Yes");
let pair = (id, gettext(id));

let s = i18n_format!("File {} not found", filename);

// translators may use "OF {1} PACKAGES, {0} DONE."
s = i18n_format!("{0} packages of {1} done.", n, total);
```
...
For [`rust-format`][] strings, the reordered syntax is `"Bar {1} {0}"`. (Using ~~`"{keywords}"`~~
does not work, and the error message gets displayed in place of the translated text.)

[`rust-format`]: https://www.gnu.org/software/gettext/manual/html_node/rust_002dformat.html

### Typescript support

The code in SLES16.1 and later contains a new `TranslatedString` type. Use it whenever your code
expects a translated text, like in component parameters.

```tsx
import { _, TranslatedString } from "~/i18n";

// a testing function which accepts a plain string
function fn1(param: string) {
  console.log(param);
}

// a testing function which accepts a TranslatedString
function fn2(param: TranslatedString) {
  console.log(param);
}

function fn3() {
  // this is allowed, passing a TranslatedString where a plain string is expected works just fine
  const translated: TranslatedString = _("foo");
  fn1(translated);

  // but passing a plain string where a TranslatedString is expected causes a compilation error
  // "Argument of type 'string' is not assignable to parameter of type 'TranslatedString'."
  const notTranslated: string = "foo";
  fn2(notTranslated);
}
```

A `TranslatedString` can be used everywhere where a plain string is accepted, but the opposite
direction is forbidden. You cannot use a plain string where a `TranslatedString` is expected.

#### Returning the string or TranslatedString type conditionally

Sometimes you need to accept both translated and not translated strings in the same function and
return the translated and not translated string depending on the input. In that case you can use
this function signature:

```tsx
// accept either a plain string or a TranslatedString argument and return the same type
function foo<T extends string | TranslatedString>(param: T): T;
```

This is used for example in the `snprintf` function:

```tsx
import { sprintf } from "sprintf-js";
import { _ } from "~/i18n";

// this returns a plain string
sprintf("foo: %s", bar);

// this returns a TranslatedString
sprintf(_("foo: %s"), bar);
```
...
- Do not use Javascript string templates, that does not work at all
  (~~``_(`User ${user} will be created`)``~~), use a string formatting function for that
  (`sprintf(_("User %s will be created"), user)`). The string will not be extracted to the POT file.
- Do not use the `_()` and `n_()` translation functions in the top level code. That is evaluated too
  early at runtime, even before loading the translations from the server. Moreover they are
  evaluated only once so they cannot react on changing the current language.
...
that only a string literal is passed to the translation functions and that the translations are not
used in the top level code. This should avoid the problems with
[missing translations](#missing-translations) mentioned above.
...
[missing translations](#missing-translations) section above but in reality the there would be no
...
2. If the text comes from back-end or the other parts check that the appropriate
   [language package](#back-end-translations) is installed.
...
4. The [agama.pot](https://github.com/agama-project/agama-weblate/blob/master/web/agama.pot) in the
   `agama-weblate` repository is up to date, if not then run the
   [Weblate Update POT](https://github.com/agama-project/agama/actions/workflows/weblate-update-pot.yml)
   Github Action manually. The same applies for the product and service POT files.
...
7. The translation files in the
   ...
   date, if not the check whether there is an
   [open pull request](https://github.com/agama-project/agama/pulls?q=is%3Aopen+is%3Apr+label%3Atranslations+label%3Abot)
   with the change, if yes then check it and merge, if not then run the
   [Weblate Merge PO](https://github.com/agama-project/agama/actions/workflows/weblate-merge-po.yml)
   GitHub Action manually, then check the created pull request. For the product translations and the
   service backend translations run the appropriate GitHub Action.
