...
translation component for each Agama part (the web front-end, the Ruby D-Bus back-end,
the HTTP Rust back-end, and product
definitions).
...
For [`rust-format`][] strings, the reordered syntax is `"Bar {1} {0}"`.
(Using ~~`"{keywords}"`~~ does not work, and the error message gets displayed
in place of the translated text.)
...
user)`). The string will not be extracted to the POT file.
...
used in the top level code. This should avoid the problems with [missing
translations](#missing-translations) mentioned above.
